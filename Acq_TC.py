...
time_step = 5 * 60
# save dir 
save_dir = r"D:\zjw\20230704_2_60XRedInit_L3strins_TimeLapse"
# Channel
...
#%%
trigger.stop_live()
# trigger.set_channel('red')
# trigger.snap(show=True)
# trigger.set_channel('green')
# trigger.snap(show=True)
# trigger.set_channel('bf')
# trigger.snap(show=True)
#%%
    ...
    for loop_i in range(loops_num):
        ...
        for p_i in range(P_num):
            ...
            img_saver.save(os.path.join(save_dir, f'fov_{p_i}'),
                           't.tiff', save_img, 'CYX', 
                           dict(DateTime=time.time(), Labels=channels_name))
        # waiting next loop
