...
class AndorSDK3Lib(object):
    ...
    def initlib(self):
        ...
        self.lib = load_lib("atcore.dll", locations=(solis_path, sdk3_path, "local", "global"), call_conv="stdcall",
                            locally=True, error_message=error_message)
        lib = self.lib
        AT_H = ctypes.c_int
        ...
        self.AT_SetEnumIndex = wrapper(lib.AT_SetEnumIndex, [AT_H, AT_pWC, ctypes.c_int],
        ...
        self.AT_GetEnumStringByIndex = wrapper(lib.AT_GetEnumStringByIndex,
                                               [AT_H, AT_pWC, ctypes.c_int, AT_pWC, ctypes.c_int],
                                               ["handle", "feature", "index", None, "length"], rvprep=[strprep],
                                               rvref=[False])
        self.AT_IsEnumIndexAvailable = wrapper(lib.AT_IsEnumIndexAvailable, [AT_H, AT_pWC, ctypes.c_int, AT_BOOL],
                                               ["handle", "feature", "value", None])
        self.AT_IsEnumIndexImplemented = wrapper(lib.AT_IsEnumIndexImplemented, [AT_H, AT_pWC, ctypes.c_int, AT_BOOL],
                                                 ["handle", "feature", "value", None])
        ...
        self.AT_GetString = wrapper(lib.AT_GetString, [AT_H, AT_pWC, AT_pWC, ctypes.c_int],
                                    ["handle", "feature", None, "length"], rvprep=[strprep], rvref=[False])
        self.AT_GetStringMaxLength = wrapper(lib.AT_GetStringMaxLength, [AT_H, AT_pWC, ctypes.c_int],
                                             ["handle", "feature", None])
        ...
        self.AT_WaitBuffer = wrapper(lib.AT_WaitBuffer, [AT_H, AT_U8p, ctypes.c_int, ctypes.c_uint],
...
try:
    nb_uint8_ro = nb.typeof(
        np.frombuffer(b"\x00", dtype="u1").reshape((1, 1)))  # for readonly attribute of a numpy array
